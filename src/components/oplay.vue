...
<div class="test_two_box">
        ...
        class="video-js"
        >
        <source :src="accObj.coursewareList[0].coursewarePath"
            type="video/mp4"
        >
        </video>
    </div>
      ...
      <div class="v-im" :class="[{'sub-center': accObj.coursewareList.length%4 == 0||accObj.coursewareList.length == 10},{'justCenter':accObj.coursewareList.length <3}]">
        <div v-if="accObj.coursewareList.length != 0" v-for="(item,index) in accObj.coursewareList" :key="index">
          <div class="view" v-if="item.type != 'mp4'" @click="toIndexbig(accObj.coursewareList,item,index)">
            <img class="img" ref="img" v-if="item.coursewarePath" :src="item.coursewarePath" alt="">
            <span class="error-tip" v-else>暂无图片</span>
          </div>
          <div v-else class="v-vo" @click='toMovie(item)'>
            <img class="img" ref="img" v-if="item.coursewareName" src="http://yd-lhy.oss-cn-beijing.aliyuncs.com/user/14ac2eea2b28b952/image/14ac2eea2b28b952&1555732145200.png">
            <span class="error-tip" v-else>暂无图片</span>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
    //   defaultImg: require('../../../static/images/default.png'),
      playerOptions: {},
      playerOption: {
     ...
     initVideo() {
      ...
      //  }
        ...
        let myPlayer = this.$video(myVideo, {
            ...
            preload: "auto",
            //设置视频播放器的显示宽度（以像素为单位）
            // width: this.accObj.width,
            width:"370px",
            // //设置视频播放器的显示高度（以像素为单位）
             height: "300px",
            // height: this.accObj.height,
        });
        },
    toMovie(item) {
